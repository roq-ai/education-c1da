            ...
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                ...
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Assignment Teacher Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_assignment_teacher_idTouser?.id}`}>
                        {data?.user_assignment_teacher_idTouser?.email}
                      </Link>
                    }
                  />
                )}
                {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="User Assignment Student Id Touser"
                    text={
                      <Link as={NextLink} href={`/users/view/${data?.user_assignment_student_idTouser?.id}`}>
                        {data?.user_assignment_student_idTouser?.email}
                      </Link>
                    }
                  />
                )}
