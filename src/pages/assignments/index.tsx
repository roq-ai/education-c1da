...
export function AssignmentListPage(props: AssignmentListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getAssignments({
        relations: ['user_assignment_teacher_idTouser', 'user_assignment_student_idTouser'],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_assignment_teacher_idTouser',
          header: 'User Assignment Teacher Id Touser',
          accessorKey: 'user_assignment_teacher_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_assignment_teacher_idTouser?.id}`}
            >
              {record.user_assignment_teacher_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_assignment_student_idTouser',
          header: 'User Assignment Student Id Touser',
          accessorKey: 'user_assignment_student_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_assignment_student_idTouser?.id}`}
            >
              {record.user_assignment_student_idTouser?.email}
            </Link>
          ),
        }
      : null,
